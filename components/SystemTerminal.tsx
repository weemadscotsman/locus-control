...
export const SystemTerminal: React.FC = () => {
  ...
  const handleRunDiagnostics = () => {
    setLogs(prev => [...prev, { timestamp: new Date().toLocaleTimeString(), message: `Running System Diagnostics...`, type: 'info' }]);
    const result = runSystemDiagnostics(config, clients);
    result.checks.forEach((check) => {
      setLogs(prev => [...prev, { timestamp: new Date().toLocaleTimeString(), message: check, type: 'info' }]);
    });
  };
    ...
    <div className="flex flex-col h-full bg-[#161920] font-mono text-xs border border-locus-border rounded overflow-hidden">
      ...
      >
        {filteredLogs.map((log, idx) => (
          <div key={idx} className={`flex gap-3 font-mono leading-tight py-0.5 border-b border-white/5 ${log.type === 'error' ? 'text-locus-error' :
